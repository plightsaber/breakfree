...
set_poses(string prmJson){
  ...
  if ((poseName == JSON_INVALID)) {
    setPoseIndex(0,FALSE);
  }
  else  {
    setPoseIndex(getPoseIndexFromName(poseName),FALSE);
  }
}
...
execute_function(string prmFunction,string prmJson){
  ...
  if ((prmFunction == "setRestraints")) {
    set_restraints(value);
  }
  else  if ((prmFunction == "tetherTo")) {
    tetherTo(value);
  }
  else  if ((prmFunction == "setPose")) {
    setPose(value,FALSE);
  }
  else  if ((prmFunction == "setPoses")) {
    set_poses(value);
  }
}
...
default {
  ...
  control(key prmAviID,integer prmLevel,integer prmEdge) {
    ...
    if ((press == CONTROL_FWD)) {
      simpleRequest("animate_mover","animation_walk_forward");
    }
    if (release) {
      simpleRequest("animate_mover","stop");
    }
  }
