...
loosen_restraint(){
  ...
  if ((!assisted())) {
    ...
    integer expEarned = 0;
    if ((activePart == "arm")) {
      (expEarned = arm_difficulty);
    }
    if ((activePart == "leg")) {
      (expEarned = leg_difficulty);
    }
    if ((activePart == "gag")) {
      (expEarned = gag_difficulty);
    }
    if ((!isArmsBound())) {
  ...
  if ((activePart == "arm")) {
    (arm_tightness -= tmpProgress);
  }
  else  if ((activePart == "leg")) {
    (leg_tightness -= tmpProgress);
  }
  else  if ((activePart == "gag")) {
    (gag_tightness -= tmpProgress);
  }
}
...
escape_action(string prmVerb){
  ...
  else  if ((prmVerb == "Squirm")) {
    (tmpAction = "2");
  }
  else  if ((prmVerb == "Struggle")) {
    (tmpAction = "3");
  }
  else  if ((prmVerb == "Thrash")) {
    (tmpAction = "4");
  }
  else  if ((prmVerb == "Pick")) {
    (tmpAction = "1");
  }
  else  if ((prmVerb == "Pluck")) {
    (tmpAction = "2");
  }
  else  if ((prmVerb == "Pull")) {
    (tmpAction = "3");
  }
  else  if ((prmVerb == "Yank")) {
    (tmpAction = "4");
  }
  ...
  if (success) {
    ...
    if (((!assisted()) && (_struggle_progress > 0))) {
      (_struggle_progress--);
    }
  }
  if ((!assisted())) {
    if (success) {
      simpleRequest("animate",(("animation_" + activePart) + "_success"));
    }
    else  {
      simpleRequest("animate",(("animation_" + activePart) + "_failure"));
    }
  }
  if (restraintFreed()) {
    return;
...
list newPuzzle(string prmRestraint){
  ...
  if ((prmRestraint == "arm")) {
    (tmpDifficulty = arm_difficulty);
  }
  if ((prmRestraint == "leg")) {
    (tmpDifficulty = leg_difficulty);
  }
  if ((prmRestraint == "gag")) {
    (tmpDifficulty = gag_difficulty);
  }
  if (assisted()) {
  }
  else  {
    integer tmpDifficultyReduction = llFloor((DEX / 2));
    if ((tmpDifficultyReduction > (tmpDifficulty / 2))) {
      (tmpDifficultyReduction = llFloor((tmpDifficulty / 2)));
    }
...
string displayTightness(string prmRestraint){
  ...
  string tmpChar = " ";
  if ((prmRestraint == "arm")) {
    (tmpTightness = arm_tightness);
  }
  if ((prmRestraint == "leg")) {
    (tmpTightness = leg_tightness);
  }
  if ((prmRestraint == "gag")) {
    (tmpTightness = gag_tightness);
  }
  if ((tmpTightness > 30)) {
    (tmpChar = "=");
    (tmpTightness -= 30);
    for ((tmpIndex = 0); (tmpIndex < tmpTightness); (tmpIndex++)) {
      (tmpDisplay += "#");
    }
  }
...
gui(integer prmScreen){
  ...
  if ((prmScreen == 0)) {
    ...
    (btn6 = "Struggle");
    (btn8 = "Thrash");
    (guiText = ("Restraint: " + ToTitle(activePart)));
...
default {
    ...
    if ((prmChannel = guiChannel)) {
      ...
      else  if ((((prmText == "<<Back>>") && (guiScreen != 0)) && ((integer)llJsonGetValue(jsonSettings,["gagOnly"])))) {
      ...
      if ((guiScreen == 0)) {
        if ((prmText == "Free Arms")) {
          get_gui("arm");
        }
        else  if ((prmText == "Free Legs")) {
          get_gui("leg");
        }
        else  if ((prmText == "Free Gag")) {
          get_gui("gag");
        }
