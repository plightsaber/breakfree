...
string defineRestraint(string name)
{
	...
	if ("Front" == name) {
		...
		restraint = llJsonSetValue(restraint, ["uid"], "frontCuff");
		restraint = llJsonSetValue(restraint, ["slot"], "wrist");
		restraint = llJsonSetValue(restraint, ["poses"], llList2Json(JSON_ARRAY, ["frontLoose"]));
		restraint = llJsonSetValue(restraint, ["attachments"], llList2Json(JSON_ARRAY, ["armCuff_wristRight", "armCuff_wristLeft", "armCuff_frontChain"]));
	} else if ("Back" == name) {
		tightness = 4;

		restraint = llJsonSetValue(restraint, ["uid"], "backCuff");
		restraint = llJsonSetValue(restraint, ["slot"], "wrist");
		restraint = llJsonSetValue(restraint, ["poses"], llList2Json(JSON_ARRAY, ["backLoose"]));
		restraint = llJsonSetValue(restraint, ["attachments"], llList2Json(JSON_ARRAY, ["armCuff_wristRight", "armCuff_wristLeft", "armCuff_backChain"]));
	}
