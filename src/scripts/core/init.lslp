...
touchUser(string prmUserObj) {
	...
	if (toucherBound && !isBound()) { llRegionSayTo(toucherID, 0, "You can't do that while bound."); return; }

	simpleRequest("resetGUI", "override");
	activeUser = toucherID;

	if (isBound()) {
		if (rpMode || (!toucherBound && toucherID == villainID)) { guiRequest("gui_bind", FALSE, activeUser, 0); }
		else { guiRequest("gui_escape", FALSE, activeUser, 0); }
	} else {
		llOwnerSay(toucherName + " is eyeing you suspiciously.");
		// TODO: Capture Dialog first
...
integer isBound() { return armsBound || legsBound || gagBound; }
...
default {
	...
	link_message(integer prmLink, integer prmValue, string prmText, key prmID) {
		...
		else if (function == "setRPMode") { rpMode = (integer)value; }
		else if (function == "resetGUI") {
			if (value == "timeout") {
				llRegionSayTo(activeUser, 0, llGetDisplayName(ownerID) + "'s menu has timed out.");
				activeUser = NULL_KEY;
			} else if (value != "override") { activeUser = NULL_KEY; }

			if (!isBound()) { villainID = NULL_KEY; }
		}
	}
