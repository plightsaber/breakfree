...
string guiText;
integer guiTimeout = 60;  // Extended timeout for leash
...
setLegTetherLength(string prmLength) {
  legTetherLength = (integer)llDeleteSubString(prmLength, -1, 2);
  tetherTo(armTargetID, "leg");
}
...
default {
  ...
  listen(integer prmChannel, string prmName, key prmID, string prmText) {
    if (prmChannel == CHANNEL_LOCKMEISTER) {
      ...
      else if (prmText == "<<Back>>") { guiRequest("gui_bind", TRUE, guiUserID, 0); }

      // Arm Tether
      if (guiScreen == 0) {
        if (prmText == "Grab") { tetherTo(guiUserID, "arm"); }
        else if (prmText == "Release") { tetherTo(NULL_KEY, "arm"); }
        else if (prmText == "Hitch") { requestHitch("arm"); return; }
        else { setArmTetherLength(prmText); }
        gui(guiScreen);
      }
    }
  }
