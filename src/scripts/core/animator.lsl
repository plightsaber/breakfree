// src.scripts.core.animator.lslp 
...
string _animation_mover_current;
...
init(){
  ...
  if (_animation_arm_base) {
    llStartAnimation(_animation_arm_base);
  }
  if ((llJsonGetValue(_pose,["animBase"]) != JSON_INVALID)) {
    llStartAnimation(llJsonGetValue(_pose,["animBase"]));
  }
  if (_mouthOpen) {
...
bindArms(string prmJson){
  llRequestPermissions(llGetOwner(),PERMISSION_TRIGGER_ANIMATION);
  if (_animation_arm_base) {
    llStopAnimation(_animation_arm_base);
  }
  if (_animation_arm_success) {
    llStopAnimation(_animation_arm_success);
  }
  if (_animation_arm_failure) {
    llStopAnimation(_animation_arm_failure);
  }
...
animate(string prmAnimation){
  ...
  if ((prmAnimation == "animation_arm_success")) {
    (animation = _animation_arm_success);
  }
  else  if ((prmAnimation == "animation_arm_failure")) {
    (animation = _animation_arm_failure);
  }
...
animate_mover(string prmAnimation){
  ...
  if (_animation_mover_current) {
    llStopAnimation(_animation_mover_current);
  }
  if ((prmAnimation == "animation_walk_forward")) {
    (_animation_mover_current = getAnimation("walk_forward"));
  }
  else  {
    (_animation_mover_current = "");
  }
  if (_animation_mover_current) {
    llStartAnimation(_animation_mover_current);
  }
}

string getAnimation(string prmAnimation){
  if ((prmAnimation == "leg_base")) {
    return llJsonGetValue(_pose,["animBase"]);
  }
  else  if ((prmAnimation == "leg_fail")) {
    return llJsonGetValue(_pose,["animFail"]);
  }
  else  if ((prmAnimation == "leg_success")) {
    return llJsonGetValue(_pose,["animSuccess"]);
  }
  else  if ((prmAnimation == "walk_forward")) {
    return llJsonGetValue(_pose,["animWalkFwd"]);
  }
  return "";
}
...
setPoseIndex(integer prmIndex){
  ...
  if ((oldAnim != newAnim)) {
    llRequestPermissions(llGetOwner(),PERMISSION_TRIGGER_ANIMATION);
    if ((oldAnim != JSON_INVALID)) {
      llStopAnimation(oldAnim);
    }
    if ((newAnim != JSON_INVALID)) {
